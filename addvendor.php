...
{ 
	...
	else
	{
		...
		else
		{
			...
			$msg="Add Vendor login is successed!";
        		          ...
        		          <form name="default_emplate" id="default_emplate" method="post" enctype="multipart/form-data" onsubmit="return stepcheck();">
        		            ...
        		            <div class='form-row'>
        		              <div class='col-xs-12 form-group' style="margin-left:0px;margin-right:0px">
        		                ...
        		                <select class='form-control' name="opportunity" size="1">
        		               	<?php 
        		            		$sql_sel_opp = "select distinct opportunity from opportunity_info;";
        							$sql_res_opp = mysqli_query($con, $sql_sel_opp) or die(mysqli_error($con));
        							while($sql_rec_opp = mysqli_fetch_assoc($sql_res_opp))
        							{									            		
        								if ($sql_rec_opp['opportunity'] != "")
        								{
        								?>
        									 <option value="<?php echo $sql_rec_opp['opportunity'];?>"><?php echo $sql_rec_opp['opportunity'];?></option>
        								<?php					
        								}													
        							}
        						?>	                    
