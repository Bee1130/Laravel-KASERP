...
if (isset($_GET['rid']))
{
	$buyer = "select * from estimates_info where auto_id=".$_GET['rid'];
	$resb = mysqli_query($con,$buyer) or die(mysqli_error($con) . "11");
	$recb = mysqli_fetch_assoc($resb);
	
	if ($recb)
	{
		$sql_previous = "SELECT auto_id FROM estimates_info WHERE auto_id<" . $recb['auto_id'] . $_SESSION['filter_user']." order by auto_id desc LIMIT 1";
		...
		$sql_next = "SELECT auto_id FROM estimates_info WHERE auto_id>" . $recb['auto_id'] .$_SESSION['filter_user']. " LIMIT 1";
...
{    
	// get old image
	$old_log = '';
	$sql_sel = sprintf("select logo from estimates_info where auto_id=%d", $_GET['rid']);		
	$res_sel = mysqli_query($con,$sql_sel) or die(mysqli_error($con));	
	if ($row_sel = mysqli_fetch_assoc($res_sel))
			$old_log = $row_sel['logo'];
	// delete record 
	$sql_del = sprintf("delete from estimates_info where auto_id=%d", $_GET['rid']);		
    ...
    if(!isset($_FILES['ImageFile']) || !is_uploaded_file($_FILES['ImageFile']['tmp_name'])){
    	if (isset($old_log) and strlen($old_log)>0)
        	$NewImageName= $old_log;
			...
			<div id="my-main-content">				
            	...
            	<div class="row" style="width: 90%;margin:auto">       
            		<div class="col-md-12 col-lg-12"> 
            			...
            			<form method="POST" action="<?php echo 'estimate.php?rid='.$_GET['rid']; ?>" enctype="multipart/form-data" id="UploadForm">            			
				                		...
				                		<div class="row table_placeholder">
					                    	<div class="table-responsive">
												<table width="97%" border="0" cellspacing="0" cellpadding="0" class="create-skus-form table" id="bnk_tbl" style="font-size:12px">
													<thead>
														<tr>														
															<th style="text-align: center;width:55%;border:0;text-align: center;vertical-align: middle;"><h1>Estimate</h1></th>
															<th style="text-align: center;width:20%;border:0"> 
																<img src="userprofile/userfiles/logos/<?php 
							                            					if (isset($_POST['logo'])) {
			                                                                    echo $_POST['logo'];
			                                                                } else if (isset($recb['logo'])) {
			                                                                    echo $recb['logo'];
			                                                                } ?>" alt="" class="img-thumbnail" >
			                                                                
							                            		 <input name="ImageFile" type="file" id="uploadFile" value="<?php if (isset($_POST['logo'])) {
			                                                                    echo $_POST['logo'];
			                                                                } else if (isset($recb['logo'])) {
			                                                                    echo $recb['logo'];
			                                                                } ?>"/>
							                            		</th>
															<th style="border:0;vertical-align: middle;"><h4><?php if (isset($_POST['addr1'])) {
			                                                                    echo $_POST['addr1'];
