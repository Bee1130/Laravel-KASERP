...
$link=mysql_connect("localhost","kaserp","") or die("Could not connect to database");
//$link=mysql_connect("localhost:3306","designcs","admin@123") or die("Could not connect to database");

// set character 
mysql_set_charset("utf8");

mysql_select_db("kaserp") or die("Could not select database");
...
function saveLog($rid,$type,$what,$action)
{
	...
	if ($type == 'Contact')
	{
		...
		$sql_res = mysql_query($sql_sel) or die(mysql_error() . "11");	
		if ($sql_rec = mysql_fetch_assoc($sql_res))
		{
			$name = $sql_rec['name'];	
		}
	...
	{		
		...
		$sql_log = sprintf("insert into log_info (log_data,log_time,agent,log_type,customer_id,url,fa_icon) values ('%s',sysdate(),'%s','%s','%d','%s','%s')",mysql_real_escape_string($log),$_SESSION['user_login'],$type,$rid,$url,$fa_icon);
