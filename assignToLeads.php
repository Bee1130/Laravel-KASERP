	...
	if (isset($_POST["assign_id"]))
 	{
 		...
 		{			
			...
			while ($recb_lead = mysqli_fetch_assoc( $resb_lead))
			{
				...
				$sql_select_worker = "select worker from assigned_worker_info";
		 		$resb_worker = mysqli_query($con, $sql_select_worker) or die(mysqli_error($con) . "11");	
				while ($recb_worker = mysqli_fetch_assoc( $resb_worker))
				{
					$workers .= ';'.trim($recb_worker['worker']);
					$notifys .= ';'.'No';
				}	
				$workers = substr($workers,1,strlen($workers)-1);
				$notifys = substr($notifys,1,strlen($workers)-1);
				if (strlen($workers)>1)
				{
